...
def _get_rag_api_url(request) -> str:
    opt = request.app.get("opt")
    return getattr(opt, "rag_api_url", "http://127.0.0.1:8000/api/chat").rstrip("/")


async def _call_rag_api(request, query: str) -> dict:
    timeout = aiohttp.ClientTimeout(total=180)
    async with aiohttp.ClientSession(timeout=timeout) as session:
        async with session.post(_get_rag_api_url(request), json={"query": query}) as resp:
            text = await resp.text()
            if resp.status >= 400:
                raise RuntimeError(f"RAG API returned HTTP {resp.status}: {text[:500]}")
            try:
                return json.loads(text)
            except json.JSONDecodeError as exc:
                raise RuntimeError(f"RAG API returned invalid JSON: {text[:500]}") from exc


def _extract_rag_answer(payload: dict) -> str:
    answer = payload.get("answer") or payload.get("response") or payload.get("text")
    if isinstance(answer, str):
        return answer.strip()
    data = payload.get("data")
    if isinstance(data, dict):
        nested_answer = data.get("answer") or data.get("response") or data.get("text")
...
async def rag_chat(request):
    """Ask Backend_RAG, then make the current avatar speak the answer."""
    try:
        params: dict = await request.json()
        ...
        rag_payload = await _call_rag_api(request, query)
        answer = _extract_rag_answer(rag_payload)
        if not answer:
            return json_error("RAG API response does not contain an answer")
        ...
        logger.exception('rag_chat route exception:')
        return json_error(str(e))


async def rag_health(request):
    """Return LiveTalking-side RAG bridge configuration."""
    return json_ok(data={"rag_api_url": _get_rag_api_url(request)})
...
def setup_routes(app):
    ...
    app.router.add_post("/rag_chat", rag_chat)
    app.router.add_get("/rag/health", rag_health)
